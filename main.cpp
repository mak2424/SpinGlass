/**
  ...
  Гибридный МК метод (HM)
  
  Программа может случайным образом генерировать обменные интегралы
  Программа может загружать обменные интегралы из файла
  
  Находит GS гибридным МК
**/
...
std::string J_file = "cell64_J0_0.dat_";//название файла с J
...
void J_read(int *J_h, int *J_v, const std::string &cell_name);
...
// функция расчета спинового избытка системы
int M_system(int *S);
...
int M_cluster2(int *cluster_state);
...
int main()
{
    ...
    J_glass_generator(J_h_host, J_v_host, J_ferr); //генератор J
    /// J_read(J_h_host, J_v_host, J_file); //J из файла
...
// расчет спинового избытка системы
int M_system(int *S)
{
    int M = 0;
    int size = n * n;
...
// функция расчета спинового избытка кластера в системе
int M_cluster(int *cluster_array, int *S)
{
    int M = 0;
    int size = cluster_size * cluster_size;
    for (int i = 0; i < size; i++)
    {
        M += S[i];
    }
...
// функция расчета спинового избытка кластера не в системе
int M_cluster2(int *cluster_state)
{
    int M = 0;
    int size = cluster_size * cluster_size;
...
void HM(int *S, int *J_h, int *J_v, int **clusters_array, int num_of_clusters)
{
            ...
            for (auto it : map_structEM)
            {
                for (auto it2 : it.second)
                {
                    ...
                    if (flag == 0)
                    {
                        if (r < (f + it2.second.p))
                        {
                            /// std::cout << std::endl << "p(" << it.first << ", " << it2.first << ") = " << it2.second.p << std::endl;
                            f += it2.second.p;
                            flag = 1;
    ...
    */
        ...
        //std::cout << T << "\t" << E_system(S, J_h, J_v) << "\t" << M_system(S) << std::endl;
        std::cout << J_file << std::endl;
        std::cout << "T=" << T << "\t" << "E=" << E_system(S, J_h, J_v) << "\t" << "M=" << M_system(S) << std::endl;
